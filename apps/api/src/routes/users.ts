...
export const users = new Elysia({ prefix: "/users" })
  .post(
    ...
    ({ body, status }) => {
      try {
        ...
        if (msg.includes("UNIQUE")) return status(409, { error: "account already exists" });
        throw e;
      }
  ...
  .post(
    ...
    ({ body, status }) => {
      ...
      if (!row) return status(404, { error: "no account for that email" });
      return {
        uid: body.uid,
  ...
  .post(
    ...
    ({ body, status }) => {
      ...
      if (!existing) return status(404, { error: "no account for that email" });
